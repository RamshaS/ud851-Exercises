...
public class GuestListAdapter extends RecyclerView.Adapter<GuestListAdapter.GuestViewHolder> {
    ...
    public void onBindViewHolder(GuestViewHolder holder, int position) {
        ...
        String tempGuestName = mCursor.getString(mCursor.getColumnIndex(WaitlistContract.WaitlistEntry.COLUMN_GUEST_NAME));

        // Completed (7) Call getInt on the cursor to get the party sizea
        int tempPartySize = mCursor.getInt(mCursor.getColumnIndex(WaitlistContract.WaitlistEntry.COLUMN_PARTY_SIZE));

        // Completed (8) Set the holder's nameTextView text to the guest's name
        holder.nameTextView.setText(tempGuestName);
        // Completed (9) Set the holder's partySizeTextView text to the party size
        holder.partySizeTextView.setText(String.valueOf(tempPartySize));
    }
